...
+(void)swizzleInstanceMethodForClass:(Class)cls origin:(SEL)origin replace:(SEL)replace{
    Method orignMethod = class_getInstanceMethod(cls, origin);
    Method replaceMethod = class_getInstanceMethod(cls, replace);
    if (class_addMethod(cls, origin, method_getImplementation(replaceMethod), method_getTypeEncoding(replaceMethod))) {
        class_replaceMethod(cls, replace, method_getImplementation(orignMethod), method_getTypeEncoding(orignMethod));
    }else{
