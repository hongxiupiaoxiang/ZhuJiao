...
+(NSMutableArray*)ReturnSortObjectArrar:(NSArray*)stringArr key:(NSString *)key{
    ...
    //获取字符串中文字的拼音首字母并与字符串共同存放
    NSMutableArray<BMChineseSort *> *chineseStringsArray=[NSMutableArray array];
    for(int i=0;i<[stringArr count];i++){
        BMChineseSort *chineseString = [[BMChineseSort alloc]init];
        //获取对象对应字段的字符串
        id temp = [stringArr objectAtIndex:i];
        NSString *str = [temp valueForKey:key];
        if (str == nil) {
    ...
    //按照拼音首字母对这些Strings进行排序
    NSSortDescriptor *des1 = [NSSortDescriptor sortDescriptorWithKey:@"pinYin" ascending:YES];
...
#pragma mark =======第三方排序修改=======
// 第三方修改
+(NSMutableArray*)IndexWithArray:(NSArray*)objectArray Key:(NSString *)key secondKey: (NSString *)secondKey{
    NSMutableArray *tempArray = [self ReturnSortObjectArrar:objectArray key:key secondKey:secondKey];
    NSMutableArray *A_Result = [NSMutableArray array];
    NSString *tempString ;
...
+(NSMutableArray*)sortObjectArray:(NSArray*)objectArray Key:(NSString *)key secondKey:(NSString *)secondKey{
...
+(NSMutableArray*)ReturnSortObjectArrar:(NSArray*)stringArr key:(NSString *)key secondKey: (NSString *)secondKey{
    ...
    for(int i=0;i<[stringArr count];i++){
        ...
        NSString *str = [temp valueForKey:key];
        if (str == nil) {
            NSString *str1 = [temp valueForKey:secondKey];
            if (str1 == nil) {
                chineseString.string = @"";
            } else {
                chineseString.string = [NSString stringWithString:[temp valueForKeyPath:secondKey]];
//                if(chineseString.string == nil){
//                    chineseString.string = @"";
//                }
            }
            ...
            chineseString.string = [NSString stringWithString:[temp valueForKeyPath:key]];
            if(chineseString.string == nil || !chineseString.string.length){
                NSString *str1 = [temp valueForKey:secondKey];
                if (str1 == nil) {
                    chineseString.string = @"";
                } else {
...
static char firstLetterArray[20903] =
